...
class LongitudinalProfile(Profile):
    ...
    def __post_init__(self):
        ...
        self._w = np.arange(DataInfo().N_CELLS_Z)
    ...
    def calc_first_moment(self) -> np.ndarray:
        ...
        """
        return DataInfo().SIZE_Z * np.dot(self._energies_per_event, self._w) / self._total_energy_per_event
    ...
    def calc_second_moment(self) -> np.ndarray:
        ...
        return np.sum(np.multiply(np.power(w * DataInfo().SIZE_Z - first_moment, 2), self._energies_per_event),
...
class LateralProfile(Profile):
    ...
    def __post_init__(self):
        ...
        self._total_energy_per_event = np.sum(self._energies_per_event, axis=1)
        self._w = np.arange(DataInfo().N_CELLS_R)
    ...
    def calc_first_moment(self) -> np.ndarray:
        ...
        """
        return DataInfo().SIZE_R * np.dot(self._energies_per_event, self._w) / self._total_energy_per_event
    ...
    def calc_second_moment(self) -> np.ndarray:
        ...
        return np.sum(np.multiply(np.power(w * DataInfo().SIZE_R - first_moment, 2), self._energies_per_event),
                      axis=1) / self._total_energy_per_event


@dataclass
class PhiProfile(Profile):
    def __post_init__(self):
        self._energies_per_event = np.sum(self._input, axis=(1, 3))
        self._total_energy_per_event = np.sum(self._energies_per_event, axis=1)
        self._w = np.arange(DataInfo().N_CELLS_PHI)
