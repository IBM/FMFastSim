...
class AE(nn.Module):
    def __init__(self, network, reg_coef = 0.0, reg_model = 'none', decoder_distribution = 'mse'):
        super().__init__()
    ...
    def forward(self, inputs):
        ...
        z0 = self.model.encoding(x_input)
        h0 = self.model.decoding(z0,c_input)

        x_out = self.gen_decoder(h0)
...
class AEHandler(ModelHandler):
    def __init__(self, gen_param, network, **kwargs):

        self._gen_param = gen_param

        self._model = AE(network=network,**gen_param)
