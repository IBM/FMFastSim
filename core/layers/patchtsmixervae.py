...
_CONFIG_FOR_DOC = "PatchTSMixerVAEConfig"


PATCHTSMIXERVAE_PRETRAINED_MODEL_ARCHIVE_LIST = []


PATCHTSMIXERVAE_START_DOCSTRING = r"""
...
PATCHTSMIXERVAE_INPUTS_DOCSTRING = r"""
    Args:
...
class PatchTSMixerVAEConfig(PretrainedConfig):
    r"""
    This is the configuration class to store the configuration of a [`PatchTSMixerVAEModel`]. It is used to instantiate a
    PatchTSMixerVAE model according to the specified arguments, defining the model architecture. Instantiating a
    configuration with the defaults will yield a similar configuration to that of the PatchTSMixerVAE {} architecture.
    ...
    Args:
        ...
        dropout (`float`, *optional*, defaults to 0.2):
            The dropout probability the `PatchTSMixerVAE` backbone. Recommended range is 0.2-0.7
        ...
        self_attn (`bool`, *optional*, defaults to `False`):
            Enable Tiny self attention across patches. This can be enabled when the output of Vanilla PatchTSMixerVAE with
        ...
        head_dropout (`float`, *optional*, defaults to 0.2):
            The dropout probability the `PatchTSMixerVAE` head.
    ...
    ```python
    >>> from transformers import PatchTSMixerVAEConfig, PatchTSMixerVAEModel

    >>> # Initializing a default PatchTSMixerVAE configuration
    >>> configuration = PatchTSMixerVAEConfig()

    >>> # Randomly initializing a model (with random weights) from the configuration
    >>> model = PatchTSMixerVAEModel(configuration)

    >>> # Accessing the model configuration
    >>> configuration = model.config
    ```"""

    model_type = "patchtsmixervae"
    attribute_map = {
        "hidden_size": "d_model",
...
# Copied from tsfm.models.tinytimemixer.modeling_tinytimemixer.TinyTimeMixerGatedAttention with TinyTimeMixer->PatchTSMixerVAE
class PatchTSMixerVAEGatedAttention(nn.Module):
    """
...
# Copied from tsfm.models.tinytimemixer.modeling_tinytimemixer.TinyTimeMixerBatchNorm with TinyTimeMixer->PatchTSMixerVAE
class PatchTSMixerVAEBatchNorm(nn.Module):
    """
    Compute batch normalization over the sequence length (time) dimension.
    """

    def __init__(self, config: PatchTSMixerVAEConfig):
        super().__init__()
...
# Copied from tsfm.models.tinytimemixer.modeling_tinytimemixer.TinyTimeMixerPositionalEncoding with TinyTimeMixer->PatchTSMixerVAE
class PatchTSMixerVAEPositionalEncoding(nn.Module):
    """
    ...
    @staticmethod
    def _init_pe(config: PatchTSMixerVAEConfig) -> nn.Parameter:
        # Positional encoding
...
# Copied from tsfm.models.tinytimemixer.modeling_tinytimemixer.TinyTimeMixerNormLayer with TinyTimeMixer->PatchTSMixerVAE
class PatchTSMixerVAENormLayer(nn.Module):
    """Normalization block

    Args:
        config (`PatchTSMixerVAEConfig`, *required*):
            Configuration.
    """
...
# Copied from tsfm.models.tinytimemixer.modeling_tinytimemixer.TinyTimeMixerMLP with TinyTimeMixer->PatchTSMixerVAE
class PatchTSMixerVAEMLP(nn.Module):
...
# Copied from tsfm.models.tinytimemixer.modeling_tinytimemixer.TinyTimeMixerChannelFeatureMixerBlock with TinyTimeMixer->PatchTSMixerVAE
class PatchTSMixerVAEChannelFeatureMixerBlock(nn.Module):
    ...
    def __init__(self, config: PatchTSMixerVAEConfig):
        ...
        self.norm = PatchTSMixerVAENormLayer(config)
        self.gated_attn = config.gated_attn
        self.mlp = PatchTSMixerVAEMLP(
        ...
        if config.gated_attn:
            self.gating_block = PatchTSMixerVAEGatedAttention(
...
# Copied from tsfm.models.tinytimemixer.modeling_tinytimemixer.TinyTimeMixerAttention with TinyTimeMixer->PatchTSMixerVAE
class PatchTSMixerVAEAttention(nn.Module):
    ...
    def __init__(
        ...
        config: Optional[PatchTSMixerVAEConfig] = None,
    ):
        super().__init__()
...
# Copied from tsfm.models.tinytimemixer.modeling_tinytimemixer.PatchMixerBlock with TinyTimeMixer->PatchTSMixerVAE
class PatchMixerBlock(nn.Module):
    ...
    def __init__(self, config: PatchTSMixerVAEConfig):
        ...
        if config.gated_attn:
            self.gating_block = PatchTSMixerVAEGatedAttention(in_size=config.num_patches, out_size=config.num_patches)

        if config.self_attn:
            self.self_attn_layer = PatchTSMixerVAEAttention(
...
# Copied from tsfm.models.tinytimemixer.modeling_tinytimemixer.FeatureMixerBlock with TinyTimeMixer->PatchTSMixerVAE
...
# Copied from tsfm.models.tinytimemixer.modeling_tinytimemixer.TinyTimeMixerLayer with TinyTimeMixer->PatchTSMixerVAE
class PatchTSMixerVAELayer(nn.Module):
    """
    The `PatchTSMixerVAE` layer that does all three kinds of mixing.
    ...
    def __init__(self, config: PatchTSMixerVAEConfig):
        ...
        if config.mode == "mix_channel":
            self.channel_feature_mixer = PatchTSMixerVAEChannelFeatureMixerBlock(config=config)
...
class PatchTSMixerVAEBlock(nn.Module):
    """The main computing framework of the `PatchTSMixerVAE` model.
    ...
    def __init__(self, config: PatchTSMixerVAEConfig):
        ...
        for i in range(num_layers):
            ...
            self.mixers.append(PatchTSMixerVAELayer(config=temp_config))
...
class PatchTSMixerVAEDecoder(nn.Module):
...
class PatchTSMixerVAEForReconstructionHead(nn.Module):
    """Reconstruction Head for Reconstruction

    Args:
        config (`PatchTSMixerVAEConfig`, *required*): Configuration.
    """
...
# Copied from tsfm.models.tinytimemixer.modeling_tinytimemixer.TinyTimeMixerPreTrainedModel with TinyTimeMixer->PatchTSMixerVAE
class PatchTSMixerVAEPreTrainedModel(PreTrainedModel):
    # Weight initialization
    config_class = PatchTSMixerVAEConfig
    ...
    def _init_weights(self, module):
        """Initialize weights"""
        if isinstance(module, PatchTSMixerVAEPositionalEncoding):
        ...
        elif isinstance(module, PatchTSMixerVAEBatchNorm):
...
# Copied from tsfm.models.tinytimemixer.modeling_tinytimemixer.TinyTimeMixerPatchify with TinyTimeMixer->PatchTSMixerVAE
class PatchTSMixerVAEPatchify(nn.Module):
    """
...
# Copied from tsfm.models.tinytimemixer.modeling_tinytimemixer.TinyTimeMixerStdScaler with TinyTimeMixer->PatchTSMixerVAE
class PatchTSMixerVAEStdScaler(nn.Module):
    """
...
# Copied from tsfm.models.tinytimemixer.modeling_tinytimemixer.TinyTimeMixerMeanScaler with TinyTimeMixer->PatchTSMixerVAE
class PatchTSMixerVAEMeanScaler(nn.Module):
    """
...
# Copied from tsfm.models.tinytimemixer.modeling_tinytimemixer.TinyTimeMixerNOPScaler with TinyTimeMixer->PatchTSMixerVAE
class PatchTSMixerVAENOPScaler(nn.Module):
    """
...
@dataclass
class PatchTSMixerVAEEncoderOutput(ModelOutput):
    """
    Base class for `PatchTSMixerVAEEncoderOutput`, with potential hidden states.
...
class PatchTSMixerVAEEncoder(PatchTSMixerVAEPreTrainedModel):
    """
    Encoder for PatchTSMixerVAE which inputs patched time-series and outputs patched embeddings.
    ...
    def __init__(self, config: PatchTSMixerVAEConfig):
        ...
        self.mlp_mixer_encoder = PatchTSMixerVAEBlock(config=config)
    ...
    @replace_return_docstrings(output_type=PatchTSMixerVAEEncoderOutput, config_class=_CONFIG_FOR_DOC)
    def forward(
        self,
        past_values: torch.Tensor,
        output_hidden_states: Optional[bool] = False,
        return_dict: Optional[bool] = None,
    ) -> Union[Tuple, PatchTSMixerVAEEncoderOutput]:
        r"""
        Args:
            past_values (`torch.FloatTensor` of shape `(batch_size, seq_length, num_input_channels)`):
        ...
        return PatchTSMixerVAEEncoderOutput(last_hidden_state=last_hidden_state, hidden_states=hidden_states)


@dataclass
class PatchTSMixerVAEModelOutput(ModelOutput):
    """
...
@add_start_docstrings(
    "The PatchTSMixerVAE Model for time-series forecasting.",
    PATCHTSMIXERVAE_START_DOCSTRING,
)
class PatchTSMixerVAEModel(PatchTSMixerVAEPreTrainedModel):
    def __init__(self, config: PatchTSMixerVAEConfig):
        ...
        self.use_return_dict = config.use_return_dict
        self.encoder = PatchTSMixerVAEEncoder(config)
        self.patching = PatchTSMixerVAEPatchify(config)

        if config.scaling == "mean":
            self.scaler = PatchTSMixerVAEMeanScaler(config)
        elif config.scaling == "std" or config.scaling is True:
            self.scaler = PatchTSMixerVAEStdScaler(config)
        else:
            self.scaler = PatchTSMixerVAENOPScaler(config)

        self.d_model = config.d_model

        self.total_embedding_size = config.d_model_layerwise[-1] * config.num_patches * config.num_input_channels
    ...
    @add_start_docstrings_to_model_forward(PATCHTSMIXERVAE_INPUTS_DOCSTRING)
    @replace_return_docstrings(output_type=PatchTSMixerVAEModelOutput, config_class=_CONFIG_FOR_DOC)
    def forward(
        self,
        past_values: torch.Tensor,
        ...
        if isinstance(encoder_output, tuple):
            encoder_output = PatchTSMixerVAEEncoderOutput(*encoder_output)
        ...
        return PatchTSMixerVAEModelOutput(
...
@dataclass
class PatchTSMixerVAEForReconstructionOutput(ModelOutput):
    """
    Output type of [`PatchTSMixerVAEForReconstructionOutput`].
...
@dataclass
class PatchTSMixerVAEDecoderWithReconstructionHeadOutput(ModelOutput):
    """
    Output type of [`PatchTSMixerVAEDecoderWithReconstructionHeadOutput`].
...
class PatchTSMixerVAEForReconstruction(PatchTSMixerVAEPreTrainedModel):
    r"""
    `PatchTSMixerVAE` for forecasting application.
    ...
    def __init__(self, config: PatchTSMixerVAEConfig):
        ...
        self.backbone = PatchTSMixerVAEModel(config)

        self.decoder_with_head = PatchTSMixerVAEDecoderWithReconstructionHead(config)
    ...
    @add_start_docstrings_to_model_forward(PATCHTSMIXERVAE_INPUTS_DOCSTRING)
    @replace_return_docstrings(output_type=PatchTSMixerVAEForReconstructionOutput, config_class=_CONFIG_FOR_DOC)
    def forward(
        self,
        past_values: torch.Tensor,
        observed_mask: Optional[torch.Tensor] = None,
        ...
        return_dict: Optional[bool] = None,
    ) -> PatchTSMixerVAEForReconstructionOutput:
        r"""
        observed_mask (`torch.FloatTensor` of shape `(batch_size, sequence_length, num_input_channels)`, *optional*):
            Boolean mask to indicate which `past_values` were observed and which were missing. Mask values selected
            in `[0, 1]`:
        ...
        if isinstance(model_output, tuple):
            model_output = PatchTSMixerVAEModelOutput(*model_output)
        ...
        if isinstance(decoder_with_head_output, tuple):
            decoder_with_head_output = PatchTSMixerVAEDecoderWithReconstructionHeadOutput(*decoder_with_head_output)
        ...
        return PatchTSMixerVAEForReconstructionOutput(
            loss=loss_val,
...
class PatchTSMixerVAEDecoderWithReconstructionHead(PatchTSMixerVAEPreTrainedModel):
    """
    Decoder + Head
    ...
    def __init__(self, config: PatchTSMixerVAEConfig):
        ...
        self.head = PatchTSMixerVAEForReconstructionHead(config)
    ...
    @replace_return_docstrings(
        output_type=PatchTSMixerVAEDecoderWithReconstructionHeadOutput, config_class=_CONFIG_FOR_DOC
    )
    def forward(
        self,
        ...
        output_hidden_states: Optional[bool] = False,
        return_dict: Optional[bool] = None,
    ) -> PatchTSMixerVAEDecoderWithReconstructionHeadOutput:
        r"""
        Args:
        ...
        return PatchTSMixerVAEDecoderWithReconstructionHeadOutput(
